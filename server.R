  ...
  if(input$go>0){
    isolate({
      if(!is.null(inFile())&&input$tagChooser!="data not loaded"){
        ...
        theChosenRows<-inFile()[inFile()[,1]%in%input$tagChooser,]
        # Order will show you The Way ChosenRows
        theChosenRows <- theChosenRows[order(theChosenRows[,2]),]
        # Convert to dates dates
        theChosenDates <- zoo(theChosenRows[,2], structure(theChosenRows[,2], class = c("POSIXt", "POSIXct")))
        ...
        binnedDates<-aggregate(x, time(x) - as.numeric(time(x)) %% 600, mean)
        ...
        #plot(theChosenDates,theChosenActions)
      }
  })
 }
  }) 
